...
describe("Email API simulator", () => {
  ...
  it("should support the primary email and webhook flows against a local server", async () => {
    ...
    const rotateDkimKeyResult = await mailchannels.emails.rotateDkimKey("example.com", "mailchannels", {
      newKey: {
    ...
    expect(listWebhooksResult.data).toContain(webhookEndpoint);
  ...
  it("should support sub-account, metrics, and suppression workflows against a local server", async () => {
    ...
    expect(createApiKeyResult.data?.value).toBeTruthy();
    ...
    expect(createSmtpPasswordResult.data?.value).toBeTruthy();
