...
describe("sendAsync", () => {
  it("should successfully queue an email", async () => {
    const mockClient = {
      post: vi.fn().mockResolvedValueOnce(fake.sendAsync.apiResponse)
    } as unknown as MailChannelsClient;

    const emails = new Emails(mockClient);
    const { data, error } = await emails.sendAsync(fake.send.options);

    expect(error).toBeNull();
    expect(data).toStrictEqual(fake.sendAsync.expectedResponse.data);
    expect(mockClient.post).toHaveBeenCalled();
  });
  ...
  it("should contain error on api response error", async () => {
    ...
    const emails = new Emails(mockClient);
    const { error } = await emails.sendAsync(fake.send.options);
